...
function makeFilterGroup(beerTagDescs) {
  ...
  for (let i = 0; i < beerTagDescs.length; i++) {
    ...
    let $ddown = $("<div>")
      .addClass("dropdown")
      .append(
        $("<button>").prop("type", "button").addClass("filter").text(tagCat)
      )
      .append($dcont)
      .appendTo($fgrp);
  ...
  $("#bsc-search")
    .autocomplete({
      source: function (request, response) {
        ...
        response(
          $.grep(beerSubCats, function (item) {
...
function updateFilter() {
  ...
  let matchingStyles = $.grep(beerSubCats, function (item) {
    ...
    for (let filter in curFilters) {
      if (item.tags.indexOf(curFilters[filter]) < 0) return false; // Any match fails: skip style
    }
    return true;
  });
          ...
          while (j < tt.length) {
            if ("th" in tt[j]) {
              ...
              for (j++; j < tt.length && "td" in tt[j]; j++) {
                tags.push({ Tag: tt[j].td[1], Meaning: tt[j].td[2] });
              }
              beerTagDescs.push({ Category: category, Tags: tags });
              continue;
            }
